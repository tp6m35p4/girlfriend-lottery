    ...
    <div class="grid grid-cols-6 mx-4 border-2 border-pink-900">
        @foreach($lotteries as $lottery)
            <div class="flex content-center justify-center items-center border-4 border-pink-900 bg-pink-800 aspect-square">
                @if($lottery->status)
                <button class="flex flex-col justify-center align-middle items-center w-full h-auto bg-yellow-400 text-sm aspect-square" wire:click="toDetail('{{ $lottery->code }}')">
                    <p>{{ $lottery->title }}</p>
                    @if($lottery->is_used)
